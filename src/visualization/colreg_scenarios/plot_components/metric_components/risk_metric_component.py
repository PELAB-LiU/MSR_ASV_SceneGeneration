...
from utils.colors import colors, light_colors
    ...
    def do_draw(self):
        ...
        for vessel in self.scenario.concrete_scene.actors:
            ...
            if self.ref_risk_vectors is not None:
                y_ref = [
                    rv.danger_sectors[vessel]
                    for rv in self.ref_risk_vectors
                    if vessel in rv.danger_sectors
                ]
                if len(y_ref) > 0:
                    x_ref = range(0, len(y_ref))
                    (line,) = self.ax.plot(
                        x_ref,
                        y_ref,
                        color=light_colors[vessel.id],
                        linestyle="-",
                        label=f"{self.scenario.get_actor_name(vessel)} no intervention",
                        linewidth=2,
                    )
            ...
            if len(y) > 0:
                ...
                (line,) = self.ax.plot(
                    x_gen,
                    y,
                    color=colors[vessel.id],
                    linewidth=1.7,
                    label=f"{self.scenario.get_actor_name(vessel)} COLREGS compliant",
                    linestyle="--",
                )
