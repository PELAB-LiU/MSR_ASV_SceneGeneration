...
class SciPyDEAlgorithm(CSPSolver):
    ...
    def init_problem(
        ...
        return (
            list(zip(logical_scenario.xl, logical_scenario.xu)),
            aggregate,
            initial_population,
        )
    ...
    def evaluate(
        self,
        some_input: Tuple[List[Tuple[int, int]], Aggregate, List[List[float]]],
        eval_data: EvaluationData,
    ):
        bounds, aggregate, initial_pop = some_input

        objective_monitor = ObjectiveMonitorCallback(
            aggregate, eval_data.timeout, self.verbose
        )
        res = differential_evolution(
            objective_monitor.objective,
            bounds,
        ...
        if self.verbose:
            print(
                f"Optimization completed in {runtime:.2f} seconds with {res.nit} iterations."
            )
            print(f"Best solution: {res.x}, Objective value: {res.fun}")
        return res, runtime

    def convert_results(
        self, some_results: Tuple[OptimizeResult, float], eval_data: EvaluationData
