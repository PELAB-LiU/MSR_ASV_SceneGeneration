...
class CSPScheduler(ABC):
    ...
    @staticmethod
    def print_status(
        seed, covered_fecs, total_fecs, evaluator_name, eval_time, max_eval_time
    ):
        print(
            f"Seed: {seed}, FECs: {covered_fecs}/{total_fecs} with {evaluator_name}. Time: {round(eval_time)}/{max_eval_time}."
        )

    @staticmethod
    def print_warmup_status(seed, warmup_number, total_warmups, evaluator_name):
        print(
            f"Seed: {seed}, Warmup {warmup_number}/{total_warmups} completed with {evaluator_name}."
        )
...
class MSRScheduler(CSPScheduler):
    def __init__(
        ...
        random_seed: int,
        warmups: int,
        average_time_per_scene: int,
        init_method: str,
    ) -> None:
        self.evaluator = evaluator
        self.scenarios: List[Tuple[LogicalScenario, FunctionalScenario]] = [
            (
                LogicalScenarioBuilder.build_from_functional(scenario, init_method),
                scenario,
            )
    ...
    def run(self, core_id: int):
        ...
        for i, (logical_scenario, functional_scenario) in enumerate(
            islice(cycle(self.scenarios), self.warmups)
        ):
            self.evaluator.evaluate(
                logical_scenario, functional_scenario, False, 0, self.max_eval_time
            )
            self.print_warmup_status(
                self.random_seed, i + 1, self.warmups, self.evaluator.name
            )
...
class OneStepScheduler(CSPScheduler):
    def __init__(
        ...
        self.actor_number = (
            self.logical_scenario.vessel_number,
            self.logical_scenario.obstacle_number,
        )
    ...
    def run(self, core_id: int):
        ...
        for i in range(self.warmups):
            self.evaluator.evaluate(
                self.logical_scenario,
                None,
                False,
                self.max_eval_time
                - GlobalConfig.FOUR_MINUTES_IN_SEC,  # Limit warmup time to avoid long waits
                self.max_eval_time,
            )
            self.print_warmup_status(
                self.random_seed, i + 1, self.warmups, self.evaluator.name
        ...
        while eval_time < self.max_eval_time and len(covered_hashes) < self.total_fecs:
            eval_data = self.evaluator.evaluate(
                self.logical_scenario, None, True, eval_time, self.max_eval_time
            )
            eval_time += eval_data.evaluation_time
            if eval_data.is_valid:
                ...
                if (
                    scenario.functional_scenario.is_relevant_by_fec
                    and fec_hash not in covered_hashes
                ):
...
class CSPSchedulerFactory:
    ...
    def factory(
        ...
        if len(scenarios) == 1 and isinstance(scenarios[0], LogicalScenario):
            return OneStepScheduler(
                evaluator, scenarios[0], random_seed, warmups, average_time_per_scene
            )
            ...
            raise ValueError(
                f"Unsupported scenario type: {type(scenarios[0])}. Expected LogicalScenario or FunctionalScenario."
            )
