...
class PyGadGAAlgorithm(CSPSolver):
    ...
    class OnGenerationCallback:
        ...
        def execute(self, ga_instance: pygad.GA):
            ...
            if elapsed_time > self.eval_data.timeout:
                if self.verbose:
                    print(
                        f"Terminating due to timeout of {self.eval_data.timeout} seconds."
                    )
                raise StopIteration
    ...
    def init_problem(
        ...
        def fitness_func(cls, solution, solution_idx):
            return Aggregate.factory(
                logical_scenario, eval_data.aggregate_strat, minimize=False
            ).evaluate(solution)[0]

        on_generation_callback = self.OnGenerationCallback(
            logical_scenario, eval_data, time.time(), self.verbose
        )
        ...
        ga_instance: pygad.GA = pygad.GA(
            ...
            gene_space=[
                {"low": low, "high": high}
                for low, high in zip(logical_scenario.xl, logical_scenario.xu)
            ],
    ...
    def evaluate(
        self,
        some_input: Tuple[pygad.GA, OnGenerationCallback],
        eval_data: EvaluationData,
    ):
        try:
    ...
    def convert_results(
        self, some_results: Tuple[pygad.GA, float], eval_data: EvaluationData
