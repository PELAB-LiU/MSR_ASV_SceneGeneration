...
class DiversityPlot(EvalPlot):
    ...
    def calculate_distribution(
        ...
        equivalence_classes: Dict[int, Tuple[ConcreteScene, int]] = (
            ConcreteSceneAbstractor.get_equivalence_class_distribution(
        ...
        processed_classes = {
            key: count
            for key, (scene, count) in equivalence_classes.items()
            if scene.is_relevant_by_fec
        }
    ...
    def aggregate_data(
        ...
        for seed in seeds:
            coverage_per_class = self.calculate_distribution(
        ...
        for i in range(
            ModelParser.TOTAL_REL_FECS[actor_numbers_by_type] - len(covered_classes)
        ):
        ...
        covered_classes = dict(
            sorted(
                covered_classes.items(), key=lambda item: np.mean(item[1]), reverse=True
            )
        )
    ...
    def create_fig(self) -> plt.Figure:
        ...
        for i, actor_number_by_type in enumerate(self.actor_numbers_by_type):
            ...
            for j, config_group in enumerate(self.comparison_groups):
                ...
                boxes: plt.BoxPlot = axi.boxplot(
                    list(values.values()),
                    positions=labels,
                    widths=0.5,
                    patch_artist=True,
                    boxprops=dict(facecolor=self.colors[config_group], color="black"),
