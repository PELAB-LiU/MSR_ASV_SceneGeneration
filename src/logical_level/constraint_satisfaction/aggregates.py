...
class Aggregate(ABC):
    ...
    def derive_penalty(self, individual: np.ndarray) -> Penalty:
        ...
        penalty = self.logical_scenario.relation_constraint.evaluate_penalty(
            assignments
        )
        return penalty
...
class ActorAggregate(Aggregate):
    ...
    def evaluate(self, individual: np.ndarray):
        ...
        return tuple(
            (
                penalty.actor_penalties[var] if var in penalty.actor_penalties else 0
                for var in self.logical_scenario.actor_variables
            )
        )
