...
import numpy as np
import scenic
from scenic.core.distributions import (RejectionException, Samplable,
                                       needsSampling)
...
def generate_scene(
    scenario: ScenicScenario, aggregate: Aggregate, first_solution, feedback=None
...
def object_to_individual(obj):
    return (
        vessel_object_to_individual(obj)
        if obj.is_vessel
        else obstacle_object_to_individual(obj)
    )


def calculate_solution(scene: ScenicScenario) -> List[float]:
    actors = sorted(
        [obj for obj in scene.objects if obj.is_actor], key=lambda obj: obj.id
    )
