...
from functional_level.metamodels.interpretation import (
    BinaryInterpretation, OSInterpretation, SeaObjectInterpretation,
    StaticObstacleInterpretation, StaticObstacleTypeInterpretation,
    TSInterpretation, VesselInterpretation, VesselTypeInterpretation,
    atVisibilityDistanceInterpretation, crossingFromPortInterpretation,
    dangerousHeadOnSectorOfInterpretation, headOnInterpretation,
    inBowSectorOfInterpretation, inPortSideSectorOfInterpretation,
    inStarboardSideSectorOfInterpretation, inSternSectorOfInterpretation,
    inVisibilityDistanceInterpretation, mayCollideInterpretation,
    outVisibilityDistanceInterpretation, overtakingToPortInterpretation,
    overtakingToStarboardInterpretation, staticObstacleTypeInterpretation,
...
class FunctionalScenario(Scenario):
    ...
    Static_obstacle_interpretation: StaticObstacleInterpretation = (
        StaticObstacleInterpretation()
    )
    Vessel_type_interpretation: VesselTypeInterpretation = VesselTypeInterpretation()
    Static_obstacle_type_interpretation: StaticObstacleTypeInterpretation = (
        StaticObstacleTypeInterpretation()
    )

    out_visibility_distance_interpretation: outVisibilityDistanceInterpretation = (
        outVisibilityDistanceInterpretation()
    )
    at_visibility_distance_interpretation: atVisibilityDistanceInterpretation = (
        atVisibilityDistanceInterpretation()
    )
    in_visibility_distance_interpretation: inVisibilityDistanceInterpretation = (
        inVisibilityDistanceInterpretation()
    )

    may_collide_interpretation: mayCollideInterpretation = mayCollideInterpretation()

    in_bow_sector_of_interpretation: inBowSectorOfInterpretation = (
        inBowSectorOfInterpretation()
    )
    in_port_side_sector_of_interpretation: inPortSideSectorOfInterpretation = (
        inPortSideSectorOfInterpretation()
    )
    in_starboard_side_sector_of_interpretation: (
        inStarboardSideSectorOfInterpretation
    ) = inStarboardSideSectorOfInterpretation()
    in_stern_sector_of_interpretation: inSternSectorOfInterpretation = (
    ...
    overtaking_to_port_interpretation: overtakingToPortInterpretation = field(
        init=False
    )
    overtaking_to_starboard_interpretation: overtakingToStarboardInterpretation = field(
        init=False
    )
    crossing_from_port_interpretation: crossingFromPortInterpretation = field(
        init=False
    )
    dangerous_head_on_sector_of_interpretation: (
        dangerousHeadOnSectorOfInterpretation
    ) = field(init=False)
    ...
    def __post_init__(self):
        ...
        object.__setattr__(
            self,
            "Vessel_interpretation",
            VesselInterpretation.union(self.OS_interpretation, self.TS_interpretation),
        )
        object.__setattr__(
            self,
            "Sea_object_interpretation",
            SeaObjectInterpretation.union(
                self.Vessel_interpretation, self.Static_obstacle_interpretation
            ),
        )
        object.__setattr__(
            self,
            "sorted_sea_objects",
            sorted(list(self.sea_objects), key=lambda x: x.id),
        )

        object.__setattr__(self, "head_on_interpretation", headOnInterpretation())
        object.__setattr__(
            self, "overtaking_to_port_interpretation", overtakingToPortInterpretation()
        )
        object.__setattr__(
            self,
            "overtaking_to_starboard_interpretation",
            overtakingToStarboardInterpretation(),
        )
        object.__setattr__(
            self, "crossing_from_port_interpretation", crossingFromPortInterpretation()
        )
        object.__setattr__(
            self,
            "dangerous_head_on_sector_of_interpretation",
    ...
    def in_masthead_sector_of(self, o1: FuncObject, o2: FuncObject) -> bool:
        return (
            self.is_sea_object(o1)
            and self.is_vessel(o2)
            and (
                self.in_port_side_sector_of_interpretation.contains((o1, o2))
                or self.in_starboard_side_sector_of_interpretation.contains((o1, o2))
            )
        )

    def at_visibility_distance_and_may_collide(
        self, o1: FuncObject, o2: FuncObject
    ) -> bool:

        return (
            self.is_sea_object(o1)
            and self.is_vessel(o2)
            and self.at_visibility_distance_interpretation.contains((o1, o2))
            and self.may_collide_interpretation.contains((o1, o2))
        )
    ...
    def out_vis_or_may_not_collide(self, o1: FuncObject, o2: FuncObject) -> bool:
        return (
            ...
            and self.is_vessel(o2)
            and (
                self.out_visibility_distance_interpretation.contains((o1, o2))
                or not self.may_collide_interpretation.contains((o1, o2))
            )
        )
    ...
    def head_on(self, o1: FuncObject, o2: FuncObject) -> bool:
        return (
            self.is_vessel(o1)
            and self.is_vessel(o2)
            and self.in_bow_sector_of_interpretation.contains((o1, o2))
            and self.in_bow_sector_of_interpretation.contains((o2, o1))
            and self.at_visibility_distance_and_may_collide(o1, o2)
        )
    ...
    def overtaking_to_port(self, o1: FuncObject, o2: FuncObject) -> bool:
        return (
            self.is_vessel(o1)
            and self.is_vessel(o2)
            and self.in_stern_sector_of_interpretation.contains((o1, o2))
            and self.in_port_side_sector_of_interpretation.contains((o2, o1))
            and self.at_visibility_distance_and_may_collide(o1, o2)
        )
    ...
    def overtaking_to_starboard(self, o1: FuncObject, o2: FuncObject) -> bool:
        return (
            ...
            and self.is_vessel(o2)
            and self.in_stern_sector_of_interpretation.contains((o1, o2))
            and self.in_starboard_side_sector_of_interpretation.contains((o2, o1))
            and self.at_visibility_distance_and_may_collide(o1, o2)
        )
    ...
    def crossing_from_port(self, o1: FuncObject, o2: FuncObject) -> bool:
        return (
            self.is_vessel(o1)
            and self.is_vessel(o2)
            and self.in_port_side_sector_of_interpretation.contains((o1, o2))
            and self.in_starboard_side_sector_of_interpretation.contains((o2, o1))
            and self.at_visibility_distance_and_may_collide(o1, o2)
            and not self.head_on(o1, o2)
        )
    ...
    def give_way(self, o: FuncObject) -> bool:
        ...
        for o2 in self.sea_objects:
            if (
                self.overtaking(o, o2)
                or self.crossing_from_port(o, o2)
                or self.head_on(o, o2)
                or self.dangerous_head_on_sector_of(o2, o)
            ):
                return True
        return False
    ...
    def in_colregs_situation_with(self, o1: FuncObject, o2: FuncObject) -> bool:
        return (
            ...
            and (
                self.head_on(o1, o2)
                or self.crossing_from_port(o1, o2)
                or self.crossing_from_port(o2, o1)
                or self.overtaking(o1, o2)
                or self.overtaking(o2, o1)
            )
        )
    ...
    def find_vessel_type_name(self, obj: FuncObject) -> Optional[str]:
        return next(
            (
                self.Vessel_type_interpretation.get_value(o2)
                for o1, o2 in self.vessel_type_interpretation
                if o1 == obj
            ),
            UnspecifiedVesselType.name,
        )

    def find_obstacle_type_name(self, obj: FuncObject) -> Optional[str]:
        return next(
            (
                self.Static_obstacle_type_interpretation.get_value(o2)
                for o1, o2 in self.static_obstacle_type_interpretation
                if o1 == obj
            ),
            UnspecifiedObstacleType.name,
        )

    @property
    def is_consistent_by_fec(self) -> bool:
        return all(
            not self.in_colregs_situation_with(o, o) for o in self.functional_objects
        ) and all(
            not (self.in_colregs_relation(o1, o2) and self.in_colregs_relation(o2, o1))
            or self.head_on(o1, o2)
            for o1, o2 in self.all_sea_object_pair_combinations
        )

    @property
    def is_relevant_by_fec(self) -> bool:
        os = self.os_object
        return self.is_consistent_by_fec and (
            all(self.in_colregs_situation_with(os, ts) for ts in self.ts_objects)
            and all(
                self.dangerous_head_on_sector_of(o, os) for o in self.obstacle_objects
            )
            and all(
                not self.in_colregs_situation_with(o1, o2)
                for (o1, o2) in self.all_ts_obstacle_pair_combinations
            )
        )
    ...
    def is_relevant_by_fsm(self) -> bool:
        ...
        return (
            all(
                self.at_visibility_distance_and_may_collide(os, ts)
                for ts in self.ts_objects
            )
            and all(
                self.dangerous_head_on_sector_of(o, os) for o in self.obstacle_objects
            )
            and all(
                self.out_vis_or_may_not_collide(o1, o2)
                for (o1, o2) in self.all_ts_obstacle_pair_combinations
            )
        )
    ...
    def is_ambiguous_by_fec(self) -> bool:
        return (
            self.is_consistent_by_fec
            and self.is_relevant_by_fec
            and self.ambiguous(self.os_object)
        )
    ...
    def all_colregs_relation_pairs(self) -> Set[Tuple[FuncObject, FuncObject]]:
        return set(
            [
                (o1, o2)
                for o1, o2 in list(permutations(self.sea_objects, 2))
                if self.in_colregs_relation(o1, o2)
            ]
        )
    ...
    def fsm_shape_hash(self, hops: int = 1) -> int:
        return self._shape_hash(
            hops,
            [
                self.in_visibility_distance_interpretation,
                self.at_visibility_distance_interpretation,
                self.out_visibility_distance_interpretation,
                self.may_collide_interpretation,
                self.in_bow_sector_of_interpretation,
                self.in_port_side_sector_of_interpretation,
                self.in_starboard_side_sector_of_interpretation,
                self.in_stern_sector_of_interpretation,
                self.static_obstacle_type_interpretation,
                self.vessel_type_interpretation,
            ],
        )
    ...
    def fec_shape_hash(self, hops: int = 1) -> int:
        return self._shape_hash(
            hops,
            [
                self.head_on_interpretation,
                self.overtaking_to_port_interpretation,
                self.overtaking_to_starboard_interpretation,
                self.crossing_from_port_interpretation,
    ...
    def _shape_hash(self, hops: int, edges: List[BinaryInterpretation]) -> int:
        ...
        neighborhood_counter: Dict[int, Dict[FuncObject, Dict[tuple, int]]] = (
            defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
        )

        def generate_tuples(
            hop: int, interpretation: BinaryInterpretation, node: FuncObject
        ) -> Set[tuple]:
        ...
        def create_attributes(hop: int, node: FuncObject) -> frozenset[tuple]:
            ...
            if self.Sea_object_interpretation.contains(node):
                attributes.add(
                    (self.Sea_object_interpretation.name_with_value(node), 1)
                )
            ...
            if self.Static_obstacle_interpretation.contains(node):
                attributes.add(
                    (self.Static_obstacle_interpretation.name_with_value(node), 1)
                )
            if self.Vessel_type_interpretation.contains(node):
                attributes.add(
                    (self.Vessel_type_interpretation.name_with_value(node), 1)
                )
            if self.Static_obstacle_type_interpretation.contains(node):
                attributes.add(
                    (self.Static_obstacle_type_interpretation.name_with_value(node), 1)
                )
        ...
        for hop in range(1, hops + 1):
            neighborhoods[hop] = {}
            for node in focused_nodes:
                neighborhoods[hop][node] = (
                    neighborhoods[hop - 1][node],
                    create_attributes(hop, node),
                )
