...
class ScenarioTypeStatisticsTable(DummyEvalPlot):
    ...
    def create_fig(self) -> plt.Figure:
        samples: Dict[Tuple[int, int], Dict[str, List[int]]] = defaultdict(
            lambda: defaultdict()
        )
        for i, actor_number_by_type in enumerate(self.actor_numbers_by_type):
            for j, comparison_group in enumerate(self.comparison_groups):
                scenarios = [
                    ConcreteSceneAbstractor.get_abstractions_from_eval(
                        eval_data
                    ).functional_scenario
                ...
                values = [
                    round(value) for value in VesselTypeSampler.sample(scenarios, 0, {})
                ]
                samples[actor_number_by_type][comparison_group] = values

            values = [
                round(value) for value in [56952 * 0.131, 56952 * 0.002, 56952 * 0.867]
            ]
            ...
            values = [
                ...
                for value in VesselTypeSampler.sample(
                    FunctionalModelManager.get_x_vessel_y_obstacle_scenarios(
                        actor_number_by_type[0], actor_number_by_type[1]
                    ),
                    0,
                    {},
                )
