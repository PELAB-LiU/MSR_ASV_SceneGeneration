...
from functional_level.metamodels.interpretation import (
    OSInterpretation, StaticObstacleInterpretation,
    StaticObstacleTypeInterpretation, TSInterpretation,
    VesselTypeInterpretation, atVisibilityDistanceInterpretation,
    ...
    def find_vessel_type_by_name(self, name: str) -> FuncObject:
        for (obj,) in self.Vessel_type_interpretation:
            if self.Vessel_type_interpretation.get_value(obj) == name:
                return obj
        raise ValueError(
            f'Vessel type "{name}" does not exist on the functional level.'
        )
    ...
    def find_obstacle_type_by_name(self, name: str) -> FuncObject:
        for (obj,) in self.Static_obstacle_type_interpretation:
            if self.Static_obstacle_type_interpretation.get_value(obj) == name:
                return obj
        raise ValueError(
            f'Obstacle type "{name}" does not exist on the functional level.'
        )
