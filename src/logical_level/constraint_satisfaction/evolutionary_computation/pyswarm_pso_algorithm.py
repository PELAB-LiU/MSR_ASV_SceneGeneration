...
class ObjectiveMonitor:
    def __init__(
        ...
        self.iter_count = 0
        self.aggregate = Aggregate.factory(
...
class PySwarmPSOAlgorithm(CSPSolver):
    ...
    def init_problem(
        ...
        optimizer = ps.single.GlobalBestPSO(
            options={"c1": eval_data.c_1, "c2": eval_data.c_2, "w": eval_data.w},
        ...
        monitor = ObjectiveMonitor(
            logical_scenario, eval_data, eval_data.timeout, self.verbose
        )
    ...
    def evaluate(
        self,
        some_input: Tuple[ps.single.GlobalBestPSO, ObjectiveMonitor],
        eval_data: EvaluationData,
    ):
        optimizer, monitor = some_input
        try:
            optimizer.optimize(
                monitor.objective, iters=np.iinfo(np.int64).max, verbose=self.verbose
            )
    ...
    def convert_results(
        self, some_results: Tuple[ObjectiveMonitor, float], eval_data: EvaluationData
