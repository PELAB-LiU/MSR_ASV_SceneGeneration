...
class RiskVectorPlot(EvalPlot):
    ...
    def create_fig(self) -> plt.Figure:
        fig, axes = plt.subplots(
            ...
            figsize=(self.vessel_num_count, 4),
            gridspec_kw={"width_ratios": [1] * self.vessel_num_count},
        ...
        for i, actor_number_by_type in enumerate(self.actor_numbers_by_type):
            ...
            for measurement, label in zip(
                self.measurements[actor_number_by_type].values(), self.group_labels
            ):
                values = [
                    self.metric_map_type(eval_data.best_scene, self.metric)
                    for eval_data in measurement
                ]
                if len(values) != 0:
                    data.append(values)
            ...
            violin_plot = axi.violinplot(
                data, widths=0.7, showmeans=True, showmedians=True
            )

            # axi.set_yticks(range(max([max(d) for d in data])))
            axi.set_xticks(range(1, len(new_group_labels) + 1), new_group_labels)
            axi.set_xticklabels(
                new_group_labels, rotation=0, ha="right", fontweight="bold"
            )
            self.set_yticks(
                axi, range(int(np.ceil(self.metric_map_max[self.metric]) + 1))
            )
            ...
            for j, group in enumerate(data, 1):  # '1' because boxplot groups start at 1
                ...
                axi.text(
                    j,
                    self.metric_map_max[self.metric] * 1.05,
                    f"{sample_size}",
                    ha="left",
                    va="center",
                    fontsize=10,
