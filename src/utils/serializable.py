...
def is_optional_type(attribute_type: Type):
    ...
    return get_origin(attribute_type) is Union and type(None) in get_args(
        attribute_type
    )
...
class Serializable(ABC):
    ...
    def from_dict(cls: Type["Serializable"], data: Dict[str, Any]) -> "Serializable":
        ...
        for attr, value in data.items():
            ...
            if value is None or not (
                isinstance(attr_type, type) and issubclass(attr_type, Serializable)
            ):
    ...
    def get_type_by_annotation(cls: Type["Serializable"], attribute_name) -> Type:
        ...
        if attribute_name not in annotations:
            raise ValueError(
                f"Attribute '{attribute_name}' is not annotated in the class."
            )
