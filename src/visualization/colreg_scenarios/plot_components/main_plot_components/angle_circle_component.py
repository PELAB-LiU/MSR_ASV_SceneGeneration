...
class AngleCircleComponent(PlotComponent):
    ...
    def __init__(
        self, ax: plt.Axes, scenario: MultiLevelScenario, linewidth=0.8, radius_ratio=10
    ) -> None:
    ...
    def one_draw(
        self, actor: ConcreteActor, state: ActorState, zorder: int, circle_color
    ):
    ...
    def draw_line(self, origin, center_vector, angle, color, length, zorder):
        ...
        (line,) = self.ax.plot(
            x, y, color=color, linewidth=self.linewidth, zorder=zorder
        )
        return line
