...
from logical_level.constraint_satisfaction.evaluation_data import \
    EvaluationData
from utils.evaluation_config import (DC_RS, DC_SB_II, DC_SB_III, MSR_CDRS,
        ...
        for i, actor_number_by_type in enumerate(self.actor_numbers_by_type):
            ...
            for measurement, label in zip(
                ...
                values = [
                    ...
                    for eval_data in list(measurement.values())[0]
                ]
                if len(values) != 0:
                    data.append(values)
                    new_group_labels.append(label)
                    print(
                        f"{actor_number_by_type[0]} vessels, {actor_number_by_type[1]} obstacles, {label}: median: {np.median(values)} , mean: {np.mean(values)}"
                    )
