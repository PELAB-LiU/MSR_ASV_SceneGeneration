...
class RelevantRatioPlot(EvalPlot):
    ...
    def config_groups(self) -> List[str]:
        return [DC_SB_II, DC_RS, MSR_SB_III, MSR_CDRS_PS]
    ...
    def create_fig(self):
        ...
        gs = gridspec.GridSpec(
            4, self.vessel_num_count, height_ratios=[4, 4, 4, 4], wspace=0, hspace=0.25
        )  # 2 rows, 6 columns
        # Top axes spans all 6 columns
        ax_relevant_fec = [
            fig.add_subplot(gs[0, i]) for i in range(self.vessel_num_count)
        ]  # Row 0, all columns
        ax_ambiguous_fec = [
            fig.add_subplot(gs[1, i]) for i in range(self.vessel_num_count)
        ]
        ax_relevant_fsm = [
            fig.add_subplot(gs[2, i]) for i in range(self.vessel_num_count)
        ]
        axt_ambiguous_fsm = [
            fig.add_subplot(gs[3, i]) for i in range(self.vessel_num_count)
        ]  # Row 1, column i
        ...
        def get_data(measurements: list[List[EvaluationData]], pred):
            ...
            print(
                [
                    f"{label} : {perc}%"
                    for label, perc in zip(new_group_labels, percentages)
                ]
            )
        ...
        for i, vessel_number in enumerate(self.actor_numbers_by_type):
            ...
            data, percentages, new_group_labels = get_data(
                self.measurements[vessel_number].values(),
                lambda eval_data: eval_data.best_scene.is_relevant_by_fec,
            )
            if len(data) == 0:
                continue

            bars: plt.BarContainer = axi.bar(
                new_group_labels,
                percentages,
                color=self.colors,
                edgecolor="black",
                linewidth=1.5,
                label=[
                    r"$\bf{" + group_label + r"}$" for group_label in self.group_labels
                ],
            )
            if i == 0:
        ...
        for i, vessel_number in enumerate(self.actor_numbers_by_type):
            ...
            axi.set_aspect("auto", adjustable="box")
            self.init_axi(
                i, axi, r"$\bf{Ambiguous}$" + "\nconsistent\n" + r"$\bf{FEC}$" + " rate"
            )

            data, percentages, new_group_labels = get_data(
                self.measurements[vessel_number].values(),
                lambda eval_data: eval_data.best_scene.is_ambiguous_by_fec,
            )
            if len(data) == 0:
                continue
        ...
        for i, vessel_number in enumerate(self.actor_numbers_by_type):
            ...
            data, percentages, new_group_labels = get_data(
                self.measurements[vessel_number].values(),
                lambda eval_data: eval_data.best_scene.is_relevant_by_fsm,
            )
            if len(data) == 0:
                continue
        ...
        for i, vessel_number in enumerate(self.actor_numbers_by_type):
            ...
            data, percentages, new_group_labels = get_data(
                self.measurements[vessel_number].values(),
                lambda eval_data: eval_data.best_scene.is_ambiguous_by_fsm,
            )
            if len(data) == 0:
                continue
