...
class AverageTimePerScenePlot(EvalPlot):
    ...
    def create_fig(self):
        ...
        gs = gridspec.GridSpec(
            3, self.vessel_num_count, height_ratios=[4.4, 4, 4], hspace=0.25
        )  # 2 rows, 6 columns
        ...
        for j, config_group in enumerate(self.comparison_groups):
            ...
            for actor_number_by_type in self.actor_numbers_by_type:
                average_runtimes.append(
                    np.mean(
                        [
                            ...
                            for eval_data in self.measurements[actor_number_by_type][
                                config_group
                            ]
                        ]
                    )
                ...
                success_rates.append(
                    np.mean(
                        [
                            1 if eval_data.is_valid else 0
                            for eval_data in self.measurements[actor_number_by_type][
                                config_group
                            ]
                        ]
            ...
            average_times_per_scene = np.array(average_runtimes) / np.array(
                success_rates
            )

            print(
                f"approach, number of vessels, success rate, average runtime, average time per scene"
            )
            for (
                vessel_num,
                success_rate,
                ...
                success_rates,
                average_runtimes,
                average_times_per_scene,
            ):
                print(
                    f"{self.group_labels[j]}, {vessel_num[0]}, {success_rate}, {average_runtime}, {average_time_per_scene}"
                )
            ...
            ax_top.plot(
                ...
                marker=".",
                linewidth=3.5,
                label=r"$\bf{" + self.group_labels[j] + r"}$",
            )
        ...
        higher_max = round(
            max_time_per_scene_value + max(max_time_per_scene_value * 0.1, 1)
        )
        ...
        for i, vessel_number in enumerate(self.actor_numbers_by_type):
            ...
            percentages = []
            new_group_labels = []
            for measurement, label in zip(
                self.measurements[vessel_number].values(), self.group_labels
            ):
                values = [
                    0 if not eval_data.is_valid else 1 for eval_data in measurement
                ]
                if len(values) != 0:
                    data.append(values)
