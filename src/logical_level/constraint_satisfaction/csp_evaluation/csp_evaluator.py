...
class CSPEvaluatorImpl(CSPEvaluator):
    ...
    def evaluate(
        ...
        try:
            ...
            eval_data.timeout = min(
                eval_data.timeout, max_eval_time - current_eval_time
            )

            gc.collect()

            initial_pop = logical_scenario.get_population(eval_data.population_size)
            some_input = self.solver.init_problem(
                logical_scenario, functional_scenario, initial_pop, eval_data
            )

            some_results = self.solver.evaluate(some_input, eval_data)

            best_solution, number_of_generations, runtime = self.solver.convert_results(
                some_results, eval_data
            )
            eval_data.evaluation_time = runtime

            assignments = Assignments(
                logical_scenario.actor_variables
            ).update_from_individual(best_solution)
            ...
            if (
                save
                and (current_eval_time + eval_data.evaluation_time)
                <= max_eval_time + 10
            ):
