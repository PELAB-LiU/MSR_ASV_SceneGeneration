    ...
    def _do_get_population(self) -> List[float]:
        ...
        for actor_variable in self.actor_variables:
            population += [
                random.uniform(x, y)
                for x, y in zip(
                    actor_variable.lower_bounds, actor_variable.upper_bounds
                )
            ]
        return population
...
class LatinHypercubeInitializer(InstanceInitializer):
    ...
    def lhs_sampling(
        self, n_samples: int, lower_bounds: List[float], upper_bounds: List[float]
    ) -> np.ndarray:
        """
        ...
        for i in range(n_dim):
            ...
            np.random.shuffle(
                points
            )  # Ensure random ordering of points in this dimension
            result[:, i] = lower_bounds[i] + points * (
    ...
    def _do_get_population(self) -> List[float]:
        population: List[float] = []
        for actor_variable in self.actor_variables:
            population += self.lhs_sampling(
                1, actor_variable.lower_bounds, actor_variable.upper_bounds
            )[0]
        return population
